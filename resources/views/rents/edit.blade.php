...
<div class="form-create">
    ...
    <form action="{{route('rents.update', $rent)}}" method="POST" class="form-container">
        ...
        <div class="form-group">
            ...
            <select id="customer_id" name="customer_id" class="form-control">
                @foreach($customers as $customer)
                    <option value="{{$customer->id}}" {{(old('customer_id', $rent->customer_id) == $customer->id) ? 'selected' : ''}}>{{$customer->name}}</option>
                @endforeach
            </select>
            @error('customer_id')
        ...
        <div class="form-group">
            ...
            <select id="dress_id" name="dress_id" class="form-control">
                @foreach($dresses as $dress)
                    <option value="{{$dress->id}}" {{(old('dress_id', $rent->dress_id) == $dress->id) ? 'selected' : ''}}>{{$dress->name}}</option>
                @endforeach
            </select>
            @error('dress_id')
                <span class="error">{{ $message }}</span>
        ...
        <button type="submit">Modificar Renta</button>
    </form>
